...
function saveCache(data) {
  try { localStorage.setItem(CACHE_KEY, JSON.stringify({ savedAt: new Date().toISOString(), data })); } catch (_) {}
}

function loadCache() {
  try { const value = localStorage.getItem(CACHE_KEY); return value ? JSON.parse(value) : null; } catch (_) { return null; }
}
...
function offlinePanel(cached) {
  if (!cached) return;
  const age = new Date(cached.savedAt);
  mountLivePanel('Offline Mode', `No live connection • cached ${age.toLocaleString('en-IN')}`, `<div class="p-4 rounded-lg bg-secondary-container/20 text-on-secondary-container">HeatShield is offline. Showing the last successfully synchronized information from this device. Live data will return automatically when the connection is restored.</div>`);
}
...
async function useMyLocation() {
  ...
  const position = await new Promise((resolve, reject) => navigator.geolocation.getCurrentPosition(resolve, reject, { enableHighAccuracy: true, timeout: 10000, maximumAge: 300000 }));
  const { latitude, longitude } = position.coords;
  return get(`/api/spatial/nearby?latitude=${latitude}&longitude=${longitude}&radius_km=5`);
}
...
window.heatshieldUseMyLocation = async function() {
  try {
    ...
    const cards = data.facilities.length ? data.facilities.map(item => `<div class="p-3 rounded-lg bg-surface-container-low border border-surface-container-high"><div class="flex justify-between"><strong>${item.name}</strong><span>${Number(item.distance_km).toFixed(2)} km</span></div><div class="text-sm text-on-surface-variant mt-1">${item.type} • ${item.available ?? (item.capacity - item.occupancy)} spaces available</div></div>`).join('') : '<div class="text-sm">No registered cooling/relief facility found within 5 km.</div>';
    mountLivePanel('Nearby Cooling Centres / Shelters', 'GPS location • PostGIS proximity search', `<div class="mb-3 text-sm text-on-surface-variant">Your location was used only for this proximity lookup.</div><div class="grid gap-2">${cards}</div>`);
  } catch (error) {
    mountLivePanel('Location unavailable', 'GPS permission or browser location failed', `<div class="p-4 rounded-lg bg-secondary-container/20">Please allow location access in your browser and try again.</div>`);
  }
};
