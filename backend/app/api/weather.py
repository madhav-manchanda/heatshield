...
def current_weather(location_id: int = Query(..., ge=1), db: Session = Depends(get_db)):
    row = db.scalar(select(Weather).where(Weather.location_id == location_id).order_by(Weather.timestamp.desc()))
    if not row:
...
def forecast(location_id: int = Query(..., ge=1), db: Session = Depends(get_db)):
    rows = db.scalars(select(Weather).where(Weather.location_id == location_id).order_by(Weather.timestamp.asc())).all()
    if not rows:
        raise HTTPException(404, "Forecast data not found")
    return [{
