...
def current_risk(location_id: int = Query(..., ge=1), db: Session = Depends(get_db)):
    row = db.scalar(select(RiskScore).where(RiskScore.location_id == location_id).order_by(RiskScore.timestamp.desc()))
    if not row:
        raise HTTPException(404, "Risk data not found")
    return row


@router.get("/forecast")
def risk_forecast(location_id: int = Query(..., ge=1), db: Session = Depends(get_db)):
    rows = db.scalars(select(RiskScore).where(RiskScore.location_id == location_id).order_by(RiskScore.timestamp.asc())).all()
    return rows
...
def risk_map(db: Session = Depends(get_db)):
    latest = {}
    rows = db.scalars(select(RiskScore).order_by(RiskScore.timestamp.desc())).all()
    for row in rows:
        latest.setdefault(row.location_id, row)
    return {"type": "FeatureCollection", "features": [_feature(row) for row in latest.values()]}
